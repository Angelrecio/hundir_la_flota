...
void borrar_coordenada(int x, int y, const char* archivo_n, int jugador) {
    ...
    while (fgets(linea, sizeof(linea), archivo) != NULL) {
        ...
        if (strstr(linea, ",") != NULL || sscanf(linea, "%*s %*s %d %d", &coordenada_x, &coordenada_y) >= 1) {
            ...
            if (coordenada_x == x && coordenada_y == y) {
                ...
                fprintf(archivo_temporal, "%d %d\n", coordenada_x, coordenada_y);
            }
        } else {
            // Otra línea del archivo, sin coordenadas
    ...
    if (borrado) {
        printf("¡El jugador %d ha eliminado una coordenada del oponente!\n", jugador);
    } else {
        printf("La coordenada especificada no coincide con ninguna coordenada existente.\n");
    }
